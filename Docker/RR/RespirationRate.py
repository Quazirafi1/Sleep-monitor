...
def on_publish(client, userdata, result):
    print("Respiration Rate Publisher : Data published.")
    pass

print('connecting to mosquitto')

client = paho.Client("RR_client")
...
csv_file_path = './RespirationRate.csv'
delimiter = ','
data_map = {}
...
try:
    with open(csv_file_path, mode='r', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            ...
            # message = f"Device 1 : Data {','.join(row)}"
            ret = client.publish("/SleepMonitor/RR", data_map_string.encode())
            print(ret.rc)
            try:
